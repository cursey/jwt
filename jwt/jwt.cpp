...
namespace jwt {
    namespace detail {
        ...
        public:
            ...
            void add(function<void()> fn) {
                m_callbacks.emplace_back(move(fn));
            }
    ...
    #define SCOPE_EXIT(x) do { onLeave.add([&]() { x; }); } while(0)
    ...
    string signHMAC(const string& str, const string& key, const string& alg) {
        ...
        HMAC(evp, key.c_str(), (int)key.length(), (const unsigned char*)str.c_str(), str.length(), out.data(), &len);
