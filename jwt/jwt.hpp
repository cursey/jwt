...
namespace jwt {
    ...
    void useAfter(nlohmann::json& payload, std::time_t time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()));

    // Adds an issued at claim ("iat").
    void issuedAt(nlohmann::json& payload, std::time_t time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()));
    ...
    struct AcceptedParameters {
        ...
        std::set<std::string> usedIDs; // ID's that will cause the JWT ID claim to fail.
